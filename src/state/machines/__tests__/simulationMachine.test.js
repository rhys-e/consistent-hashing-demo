...
describe('Simulation State Machine', () => {
  ...
  test('should complete cycle when all particles are done', async () => {
    ...
    expect(snapshot.value).toBe('awaitingTopology');
    expect(snapshot.context.cycleCount).toBe(1);

    simulationService.send({
      type: 'TOPOLOGY_READY',
      payload: {
        userRequests: snapshot.context.userRequests,
        virtualNodes: snapshot.context.virtualNodes,
        speed: snapshot.context.speed,
      },
    });
    simulationService.send({ type: 'TICK', time: now + frameTime });
    const nextSnapshot = simulationService.getSnapshot();

    expect(nextSnapshot.value).toBe('running');
    expect(snapshot.context.particleRefs).toHaveLength(3);
    expect(nextSnapshot.context.particleRefs).toHaveLength(3);
    expect(nextSnapshot.context.particleRefs.every(p => p.getSnapshot().status !== 'done')).toBe(
      true
    );
  });
