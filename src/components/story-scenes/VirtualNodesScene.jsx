 ...
 * but each server now holds ten positions rather than one, so what it lost comes
 * apart into seven pieces and both survivors take about half. 64/36 becomes 52/48.
 *
 * It is also where the story stops following individual keys. Thirty boundaries is
 * past the point where one can be tracked, and reading the ring as quantities is
  ...
  return (
    ...
    >
      <svg
        ...
        role="img"
        aria-label="Three servers on a hash ring, each splitting from one position into ten. One server then fails, and its ranges are absorbed in several pieces by both of the others rather than all by one."
        className="h-full w-full"
      >
        <SpreadRing model={SPREAD_MODEL} progress={progress} timeline={SPREAD_BEATS} />
      </svg>
    </SceneFrame>
  );
}
